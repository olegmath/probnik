...
export function Td({ children, right, mono, bold, title, style }) {
  return <td title={title} style={{ padding: '8px 10px', textAlign: right ? 'right' : 'left', fontWeight: bold ? 900 : 600, fontSize: mono ? 13 : 12, fontVariantNumeric: mono ? 'tabular-nums' : 'normal', whiteSpace: 'nowrap', borderBottom: '1px solid var(--border)', ...style }}>{children}</td>;
}
